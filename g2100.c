...
void zg_recv(U8* buf, U16* len)
{
	...
	*len = ZGSTOHS( ptr->dataLen );
	lastRssi = ZGSTOHS( ptr->rssi ); //Addedthisline
...
void enable_low_power(boolean powerSave){ // WTF?!?! Need to turn off sleepDuration? /P
    ...
    hdr[0] = ZG_INDEX_DATA_REG;
    hdr[2] = 0x00;
    hdr[1] = ZG_ENABLE_LOW_PWR_MASK;//(powerSave == true) ? ZG_ENABLE_LOW_PWR_MASK : 0x00;
    spi_transfer(hdr, 3, 1);

    if (!powerSave){ // Turned OFF power save mode, now wait until ready...
        ...
        }while((hdr[2] & ZG_ENABLE_LOW_PWR_MASK == 0));
    }else{
         //#ifdef USE_LED
         //LEDConn_off();
         //#endif
